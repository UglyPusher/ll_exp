...
Машина строится поверх единой упорядоченной append-only струны записей.
...
К струне подключён набор stateful-модулей. Каждый модуль:

- имеет собственный frontier;
- последовательно читает записи струны;
...
Опубликованная запись струны immutable. Модуль не может изменить исходное событие или уже опубликованную историю.
...
Модули поэтому продвигаются по одной и той же струне с различной скоростью. Наличие общей струны не означает наличия общей пошаговой синхронизации модулей. Глобального `tick`, на котором все модули одновременно выполняют `step()`, нет.
...
Конфигурация машины может изменяться записями той же струны.
...
> Можно ли построить детерминированную многотактовую feedback-машину поверх существующей ordered tract abstraction, не изменяя фундаментальную механику самой струны?
