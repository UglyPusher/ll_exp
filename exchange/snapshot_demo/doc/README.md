...
`RecordTape` публично использует только абсолютную позицию. Physical sequence
вычисляется persistence и stateful-модулями из их `first_sequence` и позиции.
Номер ring slot наружу не передаётся. View позиции доступен, только пока позиция уже
