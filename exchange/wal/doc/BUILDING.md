...
```powershell
cmake -S . -B ../build/wal
cmake --build ../build/wal --config Debug --target test_wal_core test_wal_slider test_wal_frontier_ring test_wal_position_view test_wal_reader test_wal_recovery
ctest --test-dir ../build/wal -C Debug -R "^test_wal_(core|slider|frontier_ring|position_view|reader|recovery)$" --output-on-failure
```
...
failure, and publication-policy behavior. The reader and recovery executables cover
